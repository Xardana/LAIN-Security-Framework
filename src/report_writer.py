"""
report_writer.py reads each task's results, then builds the PDF report and
saves a copy of the run log and every generated script. This is the only file
that knows about PDF formatting or saving files, so the rest of the program
doesn't need to worry about any of that - it just hands this file a finished
bundle of audit results.
"""

import json                              # reads/writes JSON text.
import os                                # [REQUIREMENT: Module os] path building + directory creation.
import re                                # used to spot CVE ids and clean up filenames.
from datetime import datetime, timezone  # pulls just the pieces we need out of the datetime module.
...
# The severity levels, worst to least severe.
SEVERITIES = ("critical", "high", "medium", "low", "info")
# Maps each severity name to a rank number (0 = worst), so we can sort findings by how serious they are.
SEVERITY_RANK = {name: i for i, name in enumerate(SEVERITIES)}
# Maps each severity to a color (in red, green, blue) for its badge in the PDF.
SEVERITY_RGB = {
    "critical": (138, 11, 11),
    "high": (191, 54, 12),
    "medium": (191, 130, 0),
...
# A reusable pattern that recognizes CVE ids like "CVE-2024-12345", ignoring upper/lower case.
...
def _coerce_output(script_output):
    """Whatever we were handed - a dict with "output"/"errors", a plain string,
    or nothing at all - turn it into one plain block of text, so everything
    below this point only has to deal with one type of input."""
...
def _load_json(text):
    """Try to read the text as JSON. If it doesn't work, we give back nothing
    instead of crashing, so whatever called this can decide what to do next."""
    text = text.strip()
    if not text:
        return None
    try:
        return json.loads(text)                 # try reading the whole thing as JSON
    except json.JSONDecodeError:
        pass                                    # that didn't work, try the fallback below
    """Sometimes the AI wraps its JSON answer in extra explanation text. This
    finds the first "{" and the last "}" in the text and tries reading just
    the part between them as JSON."""
    start, end = text.find("{"), text.rfind("}")
    if 0 <= start < end:                        # did we find both, in the right order?
        try:
...
def _normalize_finding(finding):
    """Take one finding from the AI and make sure it has exactly the fields
    the PDF expects, filling in safe defaults for anything missing. This way
    a messy or incomplete reply from the AI can never break the report."""
    severity = str(finding.get("severity", "info")).strip().lower()  # [REQUIREMENT: Casting]
    if severity not in SEVERITY_RANK:           # if it's not one of our known levels
        severity = "info"                       # just treat it as informational
    cves = finding.get("cves") or []            # missing/empty -> empty list
    if isinstance(cves, str):                   # if the AI gave one CVE as plain text instead of a list,
        cves = [cves]                           # wrap it in a list so we can handle it the same way
    return {
        "title": str(finding.get("title", "Untitled finding")).strip(),
        "severity": severity,
        "detail": str(finding.get("detail", "")).strip(),
        "evidence": str(finding.get("evidence", "")).strip(),
        # Keep only the CVE ids that actually look like real CVE ids, upper-cased and de-duplicated, sorted.
...
def extract_findings(script_output):
    ...
    if data is None:                            # couldn't read it as JSON
        ...
        # There was output, but it wasn't valid JSON. Rather than throw it away, show it to
        # a human as a low-priority note so nothing gets silently lost.
...
def extract_cves(script_output):
    """Collect every CVE id this task mentioned, with no duplicates."""
    cves = set()                                # a set automatically drops duplicate ids
    for finding in extract_findings(script_output):
        cves.update(finding["cves"])            # add all of this finding's CVE ids
    # Just in case the AI mentioned a CVE outside the normal "cves" field, scan the raw text for it too.
    cves.update(m.upper() for m in CVE_RE.findall(_coerce_output(script_output)))
    return sorted(cves)                         # give back a consistent, sorted list
...
def _safe(text):
    """The PDF's built-in fonts can only handle a limited set of characters,
    so this swaps out anything unsupported for a "?" instead of letting the
    program crash."""
    return str(text).encode("latin-1", "replace").decode("latin-1")


"""[REQUIREMENT: Classes] _AuditPDF is a class built on top of fpdf's own PDF
class, so it automatically gets all of fpdf's PDF-writing abilities. On top of
that, we add our own header, footer, and a handful of helper methods (h1, kv,
...) so every part of the report looks consistent."""
class _AuditPDF(FPDF):
    def __init__(self):
        # This runs when we create a new _AuditPDF. We first let fpdf set itself up normally.
        super().__init__()
        self.set_auto_page_break(auto=True, margin=18)   # start new pages automatically, with margin
        self.set_title("Defensive AI Audit Report")      # sets the PDF's title
        self._on_cover = False                  # True only while we're drawing the cover page

    def header(self):
        # fpdf calls this automatically at the top of every page. We skip it on the cover page.
        if self._on_cover:
            return                              # draw nothing on the cover page
        self.set_font("Helvetica", "I", 8)      # font, italic style, small size
        self.set_text_color(120, 120, 120)      # grey text
        self.cell(0, 8, "Defensive AI Audit Report", align="L")   # left-aligned title text
        ...
        self.line(self.l_margin, self.get_y(), self.w - self.r_margin, self.get_y())  # a thin divider line
        self.ln(4)                              # a bit of vertical space

    def footer(self):
        # fpdf calls this automatically at the bottom of every page.
        self.set_y(-15)                         # position 15mm up from the bottom of the page
        self.set_font("Helvetica", "I", 8)
        self.set_text_color(120, 120, 120)
        # Prints something like "Page 1/5", where fpdf fills in the total page count for us.
    ...
    def h2(self, text):
        # A section heading with a line underneath it.
        self.ln(2)
    ...
    def mono(self, text, size=8):
        # A fixed-width block of text (for showing prompts/JSON) so it stays neatly aligned.
    ...
    def severity_chip(self, severity):
        # Draw a small colored badge showing a finding's severity.
        r, g, b = SEVERITY_RGB.get(severity, SEVERITY_RGB["info"])
        self.set_fill_color(r, g, b)            # the badge's background color
        self.set_text_color(255, 255, 255)      # white text on top of the color
        self.set_font("Helvetica", "B", 8)
        self.cell(24, 5, _safe(severity.upper()), align="C", fill=True,  # [REQUIREMENT: Casting]
                  new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        self.set_text_color(20, 20, 20)         # reset the text color for whatever comes next


def _severity_counts(findings):
    """Count how many findings fall into each severity level, for the summary section."""
    counts = {name: 0 for name in SEVERITIES}   # start every level at 0
...
def _render_cover(pdf, audit_data):
    ...
    pdf._on_cover = True
    pdf.add_page()                              # start a fresh page
    pdf.ln(40)                                  # push the title down the page a bit
    ...
    # Print a short line for each of these, centered.
    for line in (
        ...
        pdf.multi_cell(0, 8, _safe(line), align="C",
                       new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf._on_cover = False                        # cover done; normal headers resume from here
...
def _render_profile(pdf, profile):
    ...
    # Go through each (label, key) pair and print a row for it.
    for label, key in (
    ...
    tools = profile.get("tools") or []          # list of tool names, or empty if missing
...
def _render_summary(pdf, audit_data):
    """Draw a quick at-a-glance summary before the detailed task sections."""
    ...
    # Build a line like "critical=1  high=2 ..." from the severity counts.
...
def _render_task(pdf, task):
    """Draw one section for a task: either its findings, or a clear "NOT
    COMPLETED" note so a failed task is never just silently left blank."""
    ...
    if not completed:
        pdf.set_text_color(150, 30, 30)         # red, to draw attention
    ...
    for finding in sorted(findings, key=lambda f: SEVERITY_RANK.get(f["severity"], 99)):
        ...
        pdf.multi_cell(0, 6, _safe(finding["title"]),
                       new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        if finding["detail"]:                   # only print fields that actually have content
...
def _render_cves(pdf, audit_data):
    """Show one combined list of every CVE mentioned across all tasks."""
...
def _render_appendix(pdf, audit_data):
    """Include the full details behind every task - the exact prompts, the raw
    AI reply, the validation result, and the output - so a reviewer can double-
    check exactly what was sent and received."""
    pdf.add_page()
    ...
    for task in audit_data.get("tasks", []):
        ...
        # Pretty-print the AI's response as readable JSON, capped in length so one huge
        # reply can't bloat the whole report.
...
def write_pdf_report(audit_data, output_path):
    """The main entry point for building the PDF. Builds the document section
    by section, then saves it to disk."""
    os.makedirs(os.path.dirname(output_path) or ".", exist_ok=True)  # [REQUIREMENT: Module os]

    pdf = _AuditPDF()                           # create a fresh PDF document
    pdf._title_target = _safe(audit_data.get("target", ""))   # header text shown on every page
    pdf.alias_nb_pages()                        # lets the footer show the correct total page count
    ...
    pdf.output(output_path)                     # save the finished document to disk
    return output_path
...
def _slug(value):
    """Make any value safe to use in a filename, by replacing anything that
    isn't a letter, digit, dot, underscore, or hyphen with an underscore."""
    return re.sub(r"[^A-Za-z0-9._-]", "_", str(value)) or "target"  # [REQUIREMENT: Casting]


def _timestamp():
    """A compact, sortable timestamp like "20260621T035420Z" that's also safe to use in a filename."""
    return datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")


def save_run_log(audit_data, log_dir="logs"):
    """Save a full JSON record of the whole run - the profile, every prompt,
    every reply, every validation result, and every output - as the audit trail."""
    ...
    # [REQUIREMENT: File handling] open the file for writing, and it's automatically closed afterward.
...
def save_generated_scripts(audit_data, scripts_dir="generated_scripts"):
    """Save each AI-generated script as its own .py file, so a human can review them later."""
    os.makedirs(scripts_dir, exist_ok=True)     # [REQUIREMENT: Module os] make sure the folder exists
    timestamp = _timestamp()                    # one shared timestamp for this whole batch
    saved = []                                  # the paths we wrote, to give back to the caller
    for task in audit_data.get("tasks", []):
        script = (task.get("ai_response") or {}).get("script")  # the script text, if there is one
        if not script:                          # nothing to save for this task
            continue
        # Build a filename that shows the task name and whether it passed, so failed scripts stand out.
